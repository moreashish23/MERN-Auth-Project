...
exports.signin = async (req, res) => {
    ...
    try {
        ...
        8 + 3600000), httpOnly: process.env.NODE_ENV === 'production', secure: process.env.
...
exports.sendVerificationCode = async (req, res) => {
    ...
    try {
        ...
        res.status(400).json({success:true, message:"Code Sent Failded!"})
    } catch (error) {
        console.log(error);
    }
}
    ...
    try {
        ...
        if(existingUser.verified){
            return res.status(400)
                    .json({success:false, message: "You are alread Verified!"});
        }
